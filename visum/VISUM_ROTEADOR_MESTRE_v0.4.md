<!-- VISUM ROTEADOR MESTRE v0.4 · 12-ago-2026 -->
...
Espelho — use quando a primária falhar:
https://kauewalter.com.br/visum/manifesto.json

O espelho é servido por CDN com cache e pode devolver leitura defasada
sem erro aparente. Por isso ele é alternativa, não preferência.

Ao consultar, informe em uma linha o número de `publicacao` recebido e a
fonte usada. É o que permite à pessoa perceber leitura defasada sem
comparar arquivo por arquivo.

Divergindo as duas fontes, vale a de maior `publicacao`, e registre 🔵.
Vindo o manifesto sem o campo `publicacao`, trate como leitura defasada:
consulte a outra fonte antes de concluir qualquer coisa sobre versão.
...
2. Consulte o manifesto conforme o bloco <manifesto> e compare com os
   arquivos da Base. Se estiver inacessível, registre isso em uma linha
   e siga trabalhando normalmente.
...
🔵 Informativo — duplicidade de outros tipos · nenhum arquivo PROJETO
   cadastrado · manifesto inacessível · manifesto sem o campo
   `publicacao` ou divergente entre as duas fontes.
...
instalados na Base, informando também o `publicacao` do manifesto e a
fonte consultada.
